...
#include "foye_FFT_microKernel.hpp"
#include "foye_FFT_batch_kernel.hpp"

#include "foye_FFT_radix2.hpp"
#include "foye_FFT_radix3.hpp"
#include "foye_FFT_radix4.hpp"
#include "foye_FFT_radix5.hpp"
#include "foye_FFT_radix7.hpp"
...
private:
	...
	static void append_impl(functions_map_type& map, std::index_sequence<Is...>)
	{
		(map.insert_or_assign(std::size_t{ Sizes[Is] },
			[](const float* in, float* out) { Invoker{}.template forward<Invert>(in, out, Sizes[Is]); }), ...);
	}
...
struct FFT1D_C2C_invoker
{
	...
	FFT1D_C2C_invoker(std::size_t length) : invoker(nullptr), N(length)
	{
		...
		else if (fy::fft::mix_radix::FFT1D_mixed_radix_invoker::is_processable(length))
		{
			invoker = new fy::fft::mix_radix::FFT1D_mixed_radix_invoker(length);
			return;
		}
		else if (fy::fft::rader::FFT1D_C2C_rader_invoker::is_processable(length))
		{
			invoker = new fy::fft::rader::FFT1D_C2C_rader_invoker(length);
			return;
		}
		else if (fy::fft::bluestein::FFT1D_bluestein_invoker::is_processable(length))
		{
			invoker = new fy::fft::bluestein::FFT1D_bluestein_invoker(length);
			return;
		}
		else
		{
			invoker = new fy::fft::dft::FFT1D_C2C_dft_invoker(length);
			return;
		}
	};
	...
	void dispatch(const float* input, float* output)
	{
		if (invoker)
		{
			if constexpr (invert)
			{
				invoker->backward(input, output);
			}
			else
			{
				invoker->forward(input, output);
			}
		}
		else
		{
