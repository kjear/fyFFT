...
namespace fy::fft::bluestein
{
    ...
    struct FFT1D_bluestein_invoker : basic_unsmooth_path_invoker
    {
        ...
        FFT1D_bluestein_invoker(std::size_t length) : plan(length)
        {
        ...
        {
            if (!smooth_path_invoker::is_directly_supported(plan.N))
            {
                std::complex<float>* workspace = plan.acquired_workspace();
                const std::complex<float>* kernel = plan.acquired_kernel();
                const std::complex<float>* chirp = plan.acquired_chirp_table();
                ...
                plan.forward_ptr(workspace_f, workspace_f);

                frequency_domain_convolution_complexf32<invert>(
                    reinterpret_cast<float*>(workspace),
                    reinterpret_cast<const float*>(kernel),
                    plan.M,
                    invert ? (1.0f / static_cast<float>(plan.N)) : 1.0f);

                plan.backward_ptr(workspace_f, workspace_f);
    ...
    protected:
        template<bool invert>
        void pre_process_input(const std::complex<float>* input, std::complex<float>* bufA, const std::complex<float>* table_ptr)
        {
            std::size_t n = 0;
            ...
            if (plan.M > plan.N)
            {
                std::memset(bufA + plan.N, 0, (plan.M - plan.N) * sizeof(std::complex<float>));
            }
        }

        template<bool invert>
        void post_process_output(const std::complex<float>* bufA, std::complex<float>* output, const std::complex<float>* table_ptr)
        {
            std::size_t k = 0;
