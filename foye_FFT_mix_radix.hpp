...
namespace fy::fft::mix_radix
{
    [[msvc::forceinline]] static __m256 complex_mul_avx2(__m256 data, __m256 tw)
    {
        ...
        __m256 data_swapped =
            _mm256_permute_ps(data, _MM_SHUFFLE(2, 3, 0, 1));
    ...
    struct plan_node
    {
        ...
        std::shared_ptr<aligned_vector_type<float>> twiddle_fwd = nullptr;
        std::shared_ptr<aligned_vector_type<float>> twiddle_bwd = nullptr;
    };
    ...
    static std::shared_ptr<aligned_vector_type<float>> compute_twiddles(std::size_t n)
    {
        auto ptr = std::make_shared<aligned_vector_type<float>>(n * 2);
        float* data = ptr->data();
    ...
    static std::shared_ptr<aligned_vector_type<float>> compute_twiddle_matrix(
        std::size_t N, std::size_t n1, std::size_t n2,
        std::size_t stride, const float* base)
    {
        auto mat = std::make_shared<aligned_vector_type<float>>(n1 * n2 * 2);
        float* out = mat->data();
    ...
    {
        std::size_t N;
        std::shared_ptr<plan_node> root;
        std::shared_ptr<aligned_vector_type<float>> base_twiddles;
    ...
    private:
        ...
        void populate(plan_node& node, std::size_t stride)
        {
            ...
            auto bwd = std::make_shared<aligned_vector_type<float>>(*fwd);
            for (std::size_t i = 1; i < bwd->size(); i += 2)
            {
                (*bwd)[i] = -(*bwd)[i];
            }

            node.twiddle_fwd = fwd;
            node.twiddle_bwd = bwd;

            populate(*node.sub_plan_n2, stride * node.n1);
            populate(*node.sub_plan_n1, stride * node.n2);
        }

        static std::shared_ptr<plan_node> build(std::size_t n)
        {
