...
{
    FOYE_FFT_ASSERT_ASSUME((N <= smooth_path_invoker::available_smooth_length.back()));
    if (N == 2)
    {
        return 1;
    }
...
{
    static void prepare_time_domain_convolution_kernel(
        std::size_t L,
        std::size_t N_mod,
        const std::size_t* pow_lut,
        std::complex<float>* kernel_time,
        std::complex<float>* kernel_time_inv)
    {
        const float scale_val = static_cast<float>(-2.0 * std::numbers::pi_v<double> / static_cast<double>(N_mod));
        std::size_t i = 0;
        ...
        if constexpr (is_avx2_available)
        {
            ...
            for (; i + 7 < L; i += 8)
            {
                __m256 v_idx = _mm256_setr_ps(
                    static_cast<float>(pow_lut[i + 0]), static_cast<float>(pow_lut[i + 1]),
                    static_cast<float>(pow_lut[i + 2]), static_cast<float>(pow_lut[i + 3]),
                    static_cast<float>(pow_lut[i + 4]), static_cast<float>(pow_lut[i + 5]),
                    static_cast<float>(pow_lut[i + 6]), static_cast<float>(pow_lut[i + 7])
                );

                __m256 v_angle = _mm256_mul_ps(v_idx, v_scale);
        ...
        for (; i < L; ++i)
        {
            double angle = -2.0 * std::numbers::pi_v<double> *static_cast<double>(pow_lut[i]) / static_cast<double>(N_mod);
            std::complex<float> val = { static_cast<float>(std::cos(angle)), static_cast<float>(std::sin(angle)) };

            kernel_time[i] = val;
            kernel_time_inv[i] = std::conj(val);
        }
    }
    ...
    struct plan
    {
        ...
        plan(std::size_t length) : N(length), L(length - 1), M(smooth_path_invoker::next_smooth(2 * (length - 1) - 1))
        {
            ...
            auto* ptr_lut_in = reinterpret_cast<std::int32_t*>(data_buffer.data() + offset_lut_in);
            auto* ptr_lut_out = reinterpret_cast<std::int32_t*>(data_buffer.data() + offset_lut_out);

            auto* pk_fwd = reinterpret_cast<std::complex<float>*>(data_buffer.data() + offset_k_fwd);
            auto* pk_bwd = reinterpret_cast<std::complex<float>*>(data_buffer.data() + offset_k_bwd);

            auto* ptr_g_pow = reinterpret_cast<std::size_t*>(data_buffer.data() + offset_g_pow);

            std::size_t g = find_primitive_root(N);
            ...
            auto* s_items = reinterpret_cast<scatter_item*>(data_buffer.data() + offset_scatter_items);
            for (std::size_t i = 0; i < L; ++i)
            {
            ...
            prepare_time_domain_convolution_kernel(L, N, ptr_g_pow, pk_fwd, pk_bwd);
    ...
    struct FFT1D_C2C_rader_invoker : basic_unsmooth_path_invoker
    {
        ...
        void dispatch(const float* input, float* output)
        {
            ...
            const float* kernel_ptr = reinterpret_cast<const float*>(
                invert ? plan_.acquired_backward_kernel() : plan_.acquired_forward_kernel());
